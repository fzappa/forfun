...
ND100GPS::~ND100GPS(void) { delete pPorta; }
...
void ND100GPS::setMinCont(unsigned int mcount)
{
    ...
    ND_100.c_cc[VMIN] =
        mcount ? true : false;  // VMIN is a character count ranging from 0 to 255 characters
...
void ND100GPS::getLeitura(void)
{
    ...
    do {
        ...
        if (rdlen > 0) {
            ...
            noDataTime++;
            if (noDataTime >= 5) {
                std::cerr << "[ND100GPS::getDados] Falha de comunicação com o dispositivo."
                          << std::endl;
                throw 14;
            }
        }
...
void ND100GPS::pAbrePorta(void)
{
    ...
    if (pfd < 0) {
        std::cerr << "[ND100GPS::AbrePorta] Erro ao abrir a porta " << pPorta << " : "
